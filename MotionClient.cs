...
namespace MiMotionSign
{
    public class MotionClient
    {
        ...
        private int min;
        private int max;
        private DateTime nowBeiJing;
        private string thirdName;

        public MotionClient(string User, string Pwd, int MinStep, int MaxStep, DateTime NowBeiJing)
        {
            user = User;
            password = Pwd;
            min = MinStep;
            ...
            else if (user.Length == 11 && long.TryParse(user, out _))
            {
                user = $"+86{user}";
                thirdName = "huami_phone";
            }
        ...
        public async Task<(bool success, string step, string msg)> BrushStep(CancellationToken cancellationToken)
        {
            ...
            (string app_token, string user_id, string login_error) = await Login(code, cancellationToken);
            ...
            var url = $"https://api-mifit-cn.zepp.com/v1/data/band_data.json?&t={t}";
            ...
            var data = $"userid={user_id}&last_sync_data_time=1755407692&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json={data_json}";
            ...
            return (true, $"{step}({min}~{max})", $"修改步数({step}) [{jObject?["message"]?.ToString()}]");
        }

        private async Task<(string code, string registrations_error)> Registrations(CancellationToken cancellationToken)
        {
            var url = "https://api-user.zepp.com/registrations/" + user + "/tokens";
            Dictionary<string, string> headers = new()
            {
                {"content-type", "application/x-www-form-urlencoded; charset=UTF-8"},
                {"user-agent", "MiFit/6.12.0 (MCE16; Android 16; Density/1.5)"},
            ...
            try
            {
                ...
                if (((int)response.StatusCode) == 200)
                {
                    ...
                    var ok = jObject?["ok"]?.ToString() ?? "";
                    if (ok.Equals("true", StringComparison.CurrentCultureIgnoreCase))
                    {
                        code = jObject?["access"]?.ToString() ?? "";
                        return (code, "");
                    }
                    else
                    {
                        return (code, $"用户名或密码错误，请变更后再试，status_code:{(int)response.StatusCode}");
                    }
                }
                else if (((int)response.StatusCode) == 429)
                {
                    return (code, $"请求过于频繁，请变更IP或稍后再试，status_code:{(int)response.StatusCode}");
                }
                else
                {
                    return (code, $"Access Token获取失败，status_code:{(int)response.StatusCode}");
                }
            }
        ...
        private async Task<(string app_token, string user_id, string login_error)> Login(string code, CancellationToken cancellationToken)
        {
            var url = "https://account.zepp.com/v2/client/login";
            Dictionary<string, string> headers = new()
            {
                {"content-type", "application/x-www-form-urlencoded; charset=UTF-8"},
                {"user-agent", "MiFit/6.12.0 (MCE16; Android 16; Density/1.5)"},
            ...
            if (((int)response.StatusCode) == 200)
            {
                ...
                var user_id = jObject?["token_info"]?["user_id"]?.ToString() ?? "";
                var app_token = jObject?["token_info"]?["app_token"]?.ToString() ?? "";
                return (app_token, user_id, "");
            }
            else
            {
                return ("", "", $"User ID获取失败，status_code:{(int)response.StatusCode}");
            }
        }
    }
}
